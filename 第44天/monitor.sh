...
if [ "${fileSize}" == "2.1M" ];then
  ...
  echo "$(date +%Y%m%d%H%M%S) - ${HOSTNAME} - {\"cpu\": {\"user\": ${US}, \"system\": ${SY}, \"idle\": ${ID}}, \"memory\": {\"total\": ${TOTAL}, \"free\": ${FREE}, \"percent\": ${memoryRate}}, \"disk\": {\"percent\": \"${diskRate}\"}}" |tee >>/var/log/monitor/monitors.log /var/log/monitor/monitor.log
fi
